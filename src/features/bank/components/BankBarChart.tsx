...
const BankChart = () => {
  ...
  const options = {
    responsive: true,
    plugins: { legend: { display: false }, tooltip: { callbacks: { label: (ctx: any) => `${ctx.label}: $${ctx.raw.toFixed(2)}` } } },
    scales: {
      y: { beginAtZero: true, title: { display: true, text: "Value (USD)", color: "#fff" } },
      x: { title: { display: true, text: "Tokens", color: "#fff" } },
    },
  };
    ...
    <section className="py-10 bg-[#0a0a0a] text-white">
      <div className="max-w-5xl mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-center mb-8">
          <h2 className="text-3xl font-bold mb-2">CURRENT TOKEN VALUES</h2>
          <p className="text-sm text-gray-400">Current value of tokens in Bank of BUDJU</p>
        </motion.div>
