...
// Fetch historical prices (placeholder)
const fetchHistoricalPrices = async (tokenAddress: string, days: number) => {
  return Array.from({ length: days }, (_, i) => ({
    date: new Date(Date.now() - (days - i - 1) * 24 * 60 * 60 * 1000).toISOString().split("T")[0],
    price: Math.random() * 100, // Replace with real API
  }));
};
...
// Component
const BankChart = () => {
  const [historicalData, setHistoricalData] = useState<{ [token: string]: { date: string; value: number }[] }>({});
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const historical: { [token: string]: { date: string; value: number }[] } = {};
        for (const h of holdings) {
          const prices = await fetchHistoricalPrices(h.symbol, 30);
          historical[h.symbol] = prices.map(p => ({ date: p.date, value: h.amount * p.price }));
        }
  ...
  const areaData = {
    labels: dates,
    datasets: Object.entries(historicalData).map(([symbol, data], i) => ({
      label: symbol,
  ...
  const options = {
    ...
    scales: { x: { type: "time" as const, time: { unit: "day" as const } }, y: { stacked: true, beginAtZero: true } },
  };
    ...
    <section className="py-10 bg-[#0a0a0a] text-white">
      <div className="max-w-5xl mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-center mb-8">
          <h2 className="text-3xl font-bold mb-2">STACKED TOKEN VALUES OVER TIME</h2>
          <p className="text-sm text-gray-400">Historical value of tokens in Bank of BUDJU</p>
        </motion.div>
