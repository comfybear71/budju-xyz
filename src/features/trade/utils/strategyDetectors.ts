// ============================================================
// Strategy Detectors — shared utility for detecting trading
// opportunities across all markets using technical indicators.
// Used by both SignalFeed (dashboard) and StrategySpotlight.
// ============================================================
...
export async function fetchKlines(binanceSymbol: string): Promise<CandleData[]> {
...
function detectTrendFollowing(closes: number[], price: number, base: string, symbol: string): StrategyOpportunity | null {
  ...
  if (currRsi < 25 || currRsi > 75) return null;
  ...
  return {
    strategy: "Trend Following", strategyKey: "trend_following",
...
function detectMeanReversion(closes: number[], price: number, base: string, symbol: string): StrategyOpportunity | null {
  ...
  return {
    strategy: "Mean Reversion", strategyKey: "mean_reversion",
    market: symbol, base,
...
function detectMomentumBreakout(closes: number[], price: number, base: string, symbol: string): StrategyOpportunity | null {
  ...
  return {
    strategy: "Momentum Breakout", strategyKey: "momentum",
    market: symbol, base,
    direction: isBull ? "long" : "short",
...
function detectBBSqueeze(closes: number[], price: number, base: string, symbol: string): StrategyOpportunity | null {
  ...
  return {
    strategy: "BB Squeeze", strategyKey: "bb_squeeze",
    market: symbol, base, direction,
    hotness: Math.round(Math.min(hotness, 95)),
...
function detectKeltner(closes: number[], price: number, base: string, symbol: string): StrategyOpportunity | null {
  ...
  return {
    strategy: "Keltner Channel", strategyKey: "keltner",
    market: symbol, base,
    direction: isBull ? "long" : "short",
...
function detectScalping(closes: number[], price: number, base: string, symbol: string): StrategyOpportunity | null {
  ...
  return {
    strategy: "Scalping", strategyKey: "scalping",
    market: symbol, base,
    direction: isBull ? "long" : "short",
...
function detectSRReversal(candles: CandleData[], price: number, base: string, symbol: string): StrategyOpportunity | null {
  ...
  for (const s of supports) {
    ...
    if (dist > -0.3 && dist < 0.5 && currRsi < 40) {
      return {
        strategy: "S/R Reversal", strategyKey: "sr_reversal",
        market: symbol, base, direction: "long",
        ...
        leverage: "3x", color: "emerald", icon: "🛡️",
      };
    }
  }
  ...
  for (const r of resistances) {
    ...
    if (dist > -0.3 && dist < 0.5 && currRsi > 60) {
      return {
        strategy: "S/R Reversal", strategyKey: "sr_reversal",
        market: symbol, base, direction: "short",
...
function detectGridOpportunity(closes: number[], price: number, base: string, symbol: string): StrategyOpportunity | null {
  ...
  return {
    strategy: "Grid Trading", strategyKey: "grid",
    market: symbol, base,
    ...
    leverage: "2x", color: "slate", icon: "📐",
  };
}

// ── Run All Detectors ───────────────────────────────────────

export function analyzeMarket(candles: CandleData[], base: string, symbol: string): StrategyOpportunity[] {
  const closes = candles.map(c => c.close);
...
export async function scanAllMarkets(): Promise<StrategyOpportunity[]> {
...
export const COLOR_MAP: Record<string, { bg: string; border: string; text: string; glow: string }> = {
