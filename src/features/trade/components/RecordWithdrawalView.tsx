...
import { recordWithdrawal, AUD_TO_USD } from "../services/tradeApi";
...
const RecordWithdrawalView = ({
  ...
  const estimatedSharesBurned = amountUsd > 0 ? amountUsd / currentNav : 0;
  ...
  const handleConfirm = async () => {
    ...
    const res = await recordWithdrawal(adminWallet, adminWallet, amountUsd, totalPoolValue, currency);
    ...
    if (res.success) {
      ...
      setResult({
        type: "success",
        message: `${amountLabel} → ${res.shares_burned?.toFixed(2)} shares burned at NAV $${res.nav?.toFixed(4)}`,
        shares: res.shares_burned,
        nav: res.nav,
      });
      setAmount("");
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-white/[0.06]">
        <div className="flex items-center gap-2">
          ...
          <div>
            <span className="text-base font-bold text-slate-200">Record Withdrawal</span>
            <div className="text-[10px] text-slate-500">Log your Swyftx withdrawal</div>
          </div>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="p-4 space-y-4">
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="0.00"
              className="w-full pl-10 pr-4 py-3 rounded-xl text-lg font-bold font-mono text-white bg-white/[0.04] border border-white/[0.08] placeholder:text-slate-700 focus:outline-none focus:border-red-500/40"
            />
          </div>
        ...
        {amountUsd > 0 && (
          ...
          >
            ...
            <div className="flex justify-between text-xs">
              <span className="text-slate-500">Shares to burn</span>
              <span className="text-red-400 font-bold font-mono">{estimatedSharesBurned.toFixed(2)}</span>
            </div>
            <div className="flex justify-between text-xs">
        ...
        <div className="text-[10px] text-slate-600 text-center leading-relaxed">
          Record after withdrawing AUD or USDC from Swyftx to your bank.
          Shares are burned at current NAV to keep allocations accurate.
        </div>
      </div>
    </motion.div>
  );
...
export default RecordWithdrawalView;
